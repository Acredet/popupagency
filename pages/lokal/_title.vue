          ...
          <b-container class="h-100">
            <div class="text-center h-100 d-flex flex-column flex-lg-row align-items-lg-center justify-content-lg-between">
              <h1 class="font-5">
                ...
                <b-btn v-if="$auth.loggedIn && ['admin', 'manager'].includes($auth.user.role)" variant="icon" :to="`${$t('link')}admin/listings/edit/${place.title.sv.split(' ').join('-')}`">
  ...
  async created () {
    ...
    await this.$axios.$get(`/places/${this.$route.params.title}`)
      .then(async (res) => {
        ...
        if (region.centrum) {
          const centrum = await this.$axios.get(`/centrum/${region.centrum}`)
          if (centrum.data) {
        ...
        // Increment views
        await this.$axios.patch(`/places/view/${place._id}`)
